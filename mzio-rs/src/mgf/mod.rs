...
mod test {
    ...
    fn test_reading_and_writing() {
        ...
        let reader = reader::Reader::new(mgf_file_path, 1024).unwrap();

        let entries: Vec<spectrum::Spectrum> = reader.into_fallible_iter().collect().unwrap();
        assert_eq!(entries.len(), EXPECTED_NUM_SPECTRA);

        let mut writer = writer::Writer::new(tmp_mgf_file_path).unwrap();

        writer.write_all(entries.iter()).unwrap();
        writer.flush().unwrap();

        let tmp_mgf_content = fs::read_to_string(tmp_mgf_file_path)
            .unwrap()
            .trim()
            .to_string();
